...
export class IncrementalSha256 {
  private readonly buffer = new Uint8Array(blockBytes);
  ...
  update(input: Uint8Array) {
    ...
    if (!Number.isSafeInteger(this.bytesHashed + input.byteLength)) {
    ...
    if (this.bufferLength > 0) {
      const available = blockBytes - this.bufferLength;
      const copied = Math.min(available, input.byteLength);
      this.buffer.set(input.subarray(0, copied), this.bufferLength);
      this.bufferLength += copied;
      offset += copied;
      if (this.bufferLength === blockBytes) {
        this.processBlock(this.buffer, 0);
        this.bufferLength = 0;
      }
    }

    while (offset + blockBytes <= input.byteLength) {
  ...
  digest() {
    ...
    const output = new Uint8Array(digestBytes);
  ...
  private processBlock(input: Uint8Array, offset: number) {
    const words = this.schedule;
    for (let index = 0; index < 16; index += 1) {
      const wordOffset = offset + index * 4;
      words[index] = (
        input[wordOffset] * 0x1000000 +
